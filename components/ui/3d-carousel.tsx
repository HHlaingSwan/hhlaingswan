...
const ThreeDCarousel = ({
	...
	return (
		<section
			id='ThreeDCarousel'
			className='bg-transparent py-8 min-w-full mx-auto
    flex items-center justify-center'>
			<div
				className='w-full px-4 sm:px-6 lg:px-8 
      min-w-[350px] md:min-w-[1000px]   '>
				<div
					...
					<div className='absolute top-0 left-0 w-full h-full flex items-center justify-center '>
						{items.map((item, index) => (
							<div
								key={item.id}
								className={`absolute top-0 w-full max-w-xl transform transition-all duration-500 ${getCardAnimationClass(
									index
								)}`}>
								<Card
									...
									<CardContent className='p-6 flex flex-col flex-grow'>
										...
										<div className='mt-4'>
											...
											<a
												href={item.link}
												className='text-gray-500 flex items-center  relative group'
												onClick={() => {
